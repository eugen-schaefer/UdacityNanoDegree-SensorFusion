// Functions for processing point clouds, partially from PCL lib and partially
// self-written
...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(
  ...
  typename pcl::PointCloud<PointT>::Ptr downsampled_cloud(
      new pcl::PointCloud<PointT>);
...
ProcessPointClouds<PointT>::SeparateClouds(
  ...
  extract.setIndices(inliers);
  // Extract the points in cloud referenced by inliers as a separate point
  // cloud:
...
std::pair<typename pcl::PointCloud<PointT>::Ptr,
          typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::StudentSegmentPlane(
    typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations,
    float distanceThreshold) {
  // Time segmentation process
  auto startTime = std::chrono::steady_clock::now();
  ...
  // TODO(Eugen): return earlier in case of empty cluster
  if (!cloud->empty()) {
  ...
  typename pcl::PointCloud<PointT>::Ptr cloud_inliers(
      new pcl::PointCloud<PointT>());
  typename pcl::PointCloud<PointT>::Ptr cloud_outliers(
      new pcl::PointCloud<PointT>());
...
template <typename PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::StudentClustering(
    typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance,
    int min_number_points_in_cluster) {
  // Time clustering process
  auto startTime = std::chrono::steady_clock::now();
  ...
  KdTree tree;
  tree.CreateBalancedKDTree(points, &tree.root);

  std::vector<std::vector<int>> id_clusters = euclideanCluster(
      points, &tree, clusterTolerance, min_number_points_in_cluster);

  int clusterId = 0;
  for (const auto &cluster : id_clusters) {
