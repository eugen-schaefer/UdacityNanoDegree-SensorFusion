...
TEST_F(KDTreeTest, Instantiation) {
...
TEST_F(KDTreeTest, InsertFiveNodes_Root_Left_Root_Right_Left_Right) {
    ...
    // insert the fifth point branching from root over the right then over the left to the right
        ...
        if (unit.root->right != nullptr) {
            ...
            if (unit.root->right->left != nullptr) {
                ...
                if (unit.root->right->left->right != nullptr) {
                    // Level 3
                    EXPECT_FLOAT_EQ(point5.at(0), unit.root->right->left->right->point.at(0));
                    EXPECT_FLOAT_EQ(point5.at(1), unit.root->right->left->right->point.at(1));
