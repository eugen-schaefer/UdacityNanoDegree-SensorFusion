...
void detKeypoints1() {
  ...
  // Shi-Tomasi detector
  int blockSize =
      6;       //  size of a block for computing a derivative covariation matrix over each pixel neighborhood
  ...
  for (auto it = corners.begin(); it != corners.end(); ++it) { // add corners to result vector
  ...
  detector->detect(imgGray, keypointsD, cv::Mat());
  t = ((double) cv::getTickCount() - t) / cv::getTickFrequency();
  cout << "Fast-Feature-Detector with n= " << keypointsD.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
  cv::drawKeypoints(imgGray, keypointsD, visImage);
  windowName = "Fast-Feature-Detector Results";
  cv::namedWindow(windowName, 1);
  imshow(windowName, visImage);
  cv::waitKey(0);
