...
void visualize_detector_results(const string &window_name,
...
double matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource,
  ...
  switch (matcherType) {
    case MatcherType::MAT_BF: {
      // TODO(Eugen): Enable different use of norms by providing the information
      // which detector has been used (see
...
double descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
  std::cout << descriptor_type_name << " descriptor extraction in "
            << 1000 * t / 1.0 << " ms" << endl;
  return (1000 * t / 1.0);
}
...
double detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  // visualize results
  if (is_visualization) {
    visualize_detector_results("Shi-Tomasi Corner Detector Results", keypoints,
                               img);
  }
  return (1000 * t / 1.0);
}
...
double detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  // visualize results
  if (is_visualization) {
    visualize_detector_results("Harris Corner Detector Results", keypoints,
                               img);
  }
  return (1000 * t / 1.0);
}
...
double detKeypointsFAST(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  cv::Ptr<cv::FeatureDetector> detector = cv::FastFeatureDetector::create(
      threshold, is_non_maxima_suppression, type);
