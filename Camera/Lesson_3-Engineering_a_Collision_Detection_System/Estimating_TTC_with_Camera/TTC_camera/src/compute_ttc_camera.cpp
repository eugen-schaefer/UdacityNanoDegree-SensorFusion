...
void computeTTCCamera(std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr,
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC_mean, double &TTC_median) {
  ...
  double
      medianDistRatio = (size % 2 == 0) ? (distRatios[size / 2 - 1] + distRatios[size / 2]) / 2 : distRatios[size / 2];

  double dT = 1 / frameRate;
  TTC_mean = -dT / (1 - meanDistRatio);
  TTC_median = -dT / (1 - medianDistRatio);
...
int main() {
  ...
  double TTC_mean;
  double TTC_median;
  computeTTCCamera(kptsSource, kptsRef, matches, 10.0, TTC_mean, TTC_median);
  cout << "TTC_mean = " << TTC_mean << "s" << endl;
  cout << "TTC_median = " << TTC_median << "s" << endl;
}